use crate::{
    ...
    esplora,
    esplora::Utxo,
    BTC_ASSET_ID, CHAIN, DEFAULT_SAT_PER_VBYTE, USDT_ASSET_ID,
};
use aes_gcm_siv::{
    aead::{Aead, NewAead},
    Aes256GcmSiv,
...
impl Wallet {
    ...
    pub fn get_address(&self) -> Address {
        ...
        let chain = *CHAIN.lock().expect("to be able to acquire lock");

        log::debug!("Using chain {:?} to get new address", chain);
